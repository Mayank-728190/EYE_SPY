...
def train_classifier():
    data_dir = "C:/Users/Work/Downloads/Face Recognition/data"
...
b1 = tk.Button(window, text="Training The Model", font=("Algerian", 20), bg='orange', fg='red', command=train_classifier)
    ...
    def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf):
        ...
        for (x, y, w, h) in features:
            ...
            if confidence > 77:
                ...
                if result:
                    cv2.putText(img, result[0], (x, y-5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 1, cv2.LINE_AA)
                else:
...
def generate_dataset():
    if t1.get() == "" or t2.get() == "" or t3.get() == "":
        ...
        cursor.execute("INSERT INTO Users (name, age, address) VALUES (%s, %s, %s)", (name, age, address))
        db.commit()
        ...
        while True:
            ...
            if ret:
                ...
                if cropped_face is not None:
                    ...
                    cursor.execute("UPDATE Users SET img_path = %s WHERE id = %s", (file_name_path, user_id))
                    db.commit()
                    
                    if cv2.waitKey(4) == 13 or img_id == 40:
                        break
                else:
